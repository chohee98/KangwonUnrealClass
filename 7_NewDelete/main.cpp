...
int main()
{
	/*
	//정적 메모리 할당
	//정적으로 메모리를 할당하면 컴파일 시 Stack 영역에 할당되며, 함수를 빠져가날때 소멸된다.
	//프로그램 컴파일 시 Stack에 얼마만큰의 크기로 할당을 해야하는지 결정되기 때문에 컴파일 이후 크기를 변경할 수 없다.
	...
	//new & delete
	//new연산자로 생성자 반드시 delete 연산자로 해제 해주어야 한다.
	//new 연산자로 배열 생성시 delete[] 연산자로 배열임을 명시 해주어야 한다.

	int* pA = new int(0);	//Cpp에서 동적 할당하는 기능이 추가됨
	int* pArr = new int[10];
	...
	*/
	...
	a = b; //얕은 복사: a의 주소값에 b의 주소값을 넣는다. b가 가리키고 있는걸 a도 가리키게 하겠다. a가 가리키던 값이 붕 떠버림.
